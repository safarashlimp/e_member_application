  ...
  @override
    ...
    AddFamilyMemberViewModel _buildViewModel() {
      ...
      bloodgroup: int.tryParse(selectedBloodGroupId ?? '0'),
  casteId: int.tryParse(selectedCasteId ?? '0'),
  maritalStatusId: int.tryParse(selectedMaritalStatusId ?? '0'),
  ...
  // agricultureType: agricultureType?.toString(), // or null
  // insuranceTypeId: insuranceTypeId?.toString(), 

      // Education
      qualificationId: selectedQualificationId,
      currentlyStudying: student = 0,
      courseId: int.tryParse(selectedEducationId ?? '0'), // ✅ convert to int
      courseOther: courseStudy.text,
      institution: studyCenter.text,
      needEducationSupport: needEducationHelp =0,

      // Health
      diseases: int.tryParse(hasHealthIssuesId ?? '0'), // ✅ convert to int

      // Welfare
      pensionTypeId: int.tryParse(selectedPensionTypeId ?? '0'), // ✅ convert to int
      includedInRation: isIncludedInRationCard = 0,
      receivingPension: isPensionReceiving = 0,
      needPensionTypeId: isPensionRequiredId,
      povertyPgm: povertyProgramMap[selectedProvertyPrgm] ?? 0,

      // Employment, Survey, other fields...
      ...
      agricultureType:int.tryParse (farmingTypeId ?? '0'),
      isPatient: patient  = 0,
      treatmentPlace: treatmentPlaceLabel.text,
      disabled: hasDisability = 0,
      disabilityBenefit: disabilityBenefit = 0,
      insuranceCard: healthInsuranceCard =0,
      insuranceTypeId:int.tryParse( healthInsuranceId ?? '0'),
                                          ...
                                          onChanged: (value) {
                                            setState(() {
                                              ...
                                              final selectedItem = state.items
                                                  .firstWhere(
                                                      (e) => e.name == value);
                                              selectedGenderId = selectedItem.id;
                                            });
                                          },
                                        );
                                      }
