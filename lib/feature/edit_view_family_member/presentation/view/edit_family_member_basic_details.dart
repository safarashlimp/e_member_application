...
//         ),
//       ],
//       child: _allDataLoaded ? _buildForm() 
...
class _EditFamilyMemberBasicDetailsState
  ...
  // GlobalKeys for scroll-to-widget
  final GlobalKey familyNameKey = GlobalKey();
  final GlobalKey mobileKey = GlobalKey();
  final GlobalKey whatsappKey = GlobalKey();
  final GlobalKey bloodGroupKey = GlobalKey();
  ...
  @override
  Widget build(BuildContext context) {
    // ✅ REMOVED: MultiBlocListener and _allDataLoaded check
    // Now form shows immediately, dropdowns load independently
    return _buildForm();
  }
                      ...
                      SurveySection(
                        ...
                        child: Column(
                          children: [
                            ...
                            // ✅ Blood Group Dropdown - loads independently
                            BlocBuilder<BloodGroupBloc, BloodGroupState>(
                              builder: (context, state) {
                                if (state is BloodGroupLoading) {
                                  return _buildLoadingDropdown('* രക്തഗ്രൂപ്പ്');
                                }
                                ...
                                if (state is BloodGroupError) {
                                  return _buildErrorDropdown('* രക്തഗ്രൂപ്പ്', state.message);
                                }
                            ...
                            // ✅ Relation Dropdown - loads independently
                            BlocBuilder<RelationDropBloc, RelationDropState>(
                              builder: (context, state) {
                                if (state is RelationDropLoading) {
                                  return _buildLoadingDropdown('* കുടുംബനാഥനുമായുള്ള ബന്ധം');
                                }
                                ...
                                if (state is RelationDropError) {
                                  return _buildErrorDropdown('* കുടുംബനാഥനുമായുള്ള ബന്ധം', state.message);
                                }
                            ...
                            // ✅ Gender Dropdown - loads independently
                            BlocBuilder<GenderBloc, GenderState>(
                              builder: (context, state) {
                                if (state is GenderLoading) {
                                  return _buildLoadingDropdown('* ലിംഗം');
                                }
                                ...
                                if (state is GenderError) {
                                  return _buildErrorDropdown('* ലിംഗം', state.message);
                                }
                                  ...
                                  child: BlocBuilder<MaritalStatusBloc, MaritalStatusState>(
                                    builder: (context, state) {
                                      if (state is MaritalStatusLoading) {
                                        return _buildLoadingDropdown('വിവാഹസ്ഥിതി');
                                      }
                                      ...
                                      if (state is MaritalStatusLoaded) {
                                        ...
                                        return AppDropdownField<String>(
                                          ...
                                          onChanged: (value) {
                                            setState(() {
                                              selectedMaritalStatus = value;
                                              selectedMaritalStatusId = state.items
                                                  .firstWhere((e) => e.name == value)
                                                  .id;
                                            });
                                          },
                                      ...
                                      if (state is MaritalStatusError) {
                                        return _buildErrorDropdown('വിവാഹസ്ഥിതി', state.message);
                                      }
                            ...
                            BlocBuilder<ReligionBloc, ReligionState>(
                              builder: (context, state) {
                                if (state is ReligionLoading) {
                                  return _buildLoadingDropdown('മതം');
                                }
                                ...
                                if (state is ReligionError) {
                                  return _buildErrorDropdown('മതം', state.message);
                                }
                            ...
                            BlocBuilder<CasteBloc, CasteState>(
                              builder: (context, state) {
                                if (state is CasteLoading) {
                                  return _buildLoadingDropdown('ജാതി');
                                }
                                ...
                                if (state is CasteError) {
                                  return _buildErrorDropdown('ജാതി', state.message);
                                }
  ...
  // ✅ Helper widget for loading dropdown
  ...
  // ✅ Helper widget for error dropdown
  ...
  void _handleSubmit() {
    if (familyMemberName.text.trim().isEmpty) {
      showSnack(context, "കുടുംബാംഗത്തിന്റെ പേര് നൽകുക");
      _scrollToField(familyNameKey, focusNode: familyNameFocus);
      return;
    }
    if (mobileNumber.text.trim().isEmpty) {
      showSnack(context, "മൊബൈൽ നമ്പർ നൽകുക");
      _scrollToField(mobileKey, focusNode: mobileFocus);
      return;
    }
    if (!RegExp(r'^[0-9]{10}$').hasMatch(mobileNumber.text.trim())) {
      showSnack(context, "മൊബൈൽ നമ്പർ 10 അക്കമായിരിക്കണം");
      _scrollToField(mobileKey, focusNode: mobileFocus);
      return;
    }
    if (whatsupNumber.text.trim().isEmpty) {
      showSnack(context, "വാട്സ്ആപ്പ് നമ്പർ നൽകുക");
      _scrollToField(whatsappKey, focusNode: whatsappFocus);
      return;
    }
    if (!RegExp(r'^[0-9]{10}$').hasMatch(whatsupNumber.text.trim())) {
      showSnack(context, "വാട്സ്ആപ്പ് നമ്പർ 10 അക്കമായിരിക്കണം");
      _scrollToField(whatsappKey, focusNode: whatsappFocus);
      return;
    }
    if (selectedBloodGroup == null || selectedBloodGroup!.isEmpty) {
      showSnack(context, "രക്തഗ്രൂപ്പ് തിരഞ്ഞെടുക്കുക");
      _scrollToField(bloodGroupKey);
      return;
    }
    if (selectedReletion == null || selectedReletion!.isEmpty) {
      showSnack(context, "കുടുംബനാഥനുമായുള്ള ബന്ധം തിരഞ്ഞെടുക്കുക");
      _scrollToField(relationKey);
      return;
    }
    ...
    if (surveyorNameLabel.text.trim().isEmpty) {
      showSnack(context, "സർവേ നടത്തിയ ആളുടെ പേര് നൽകുക");
